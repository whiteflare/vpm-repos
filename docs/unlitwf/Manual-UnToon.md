...
#### ANTI_SHADE(アンチシャドウマスク) {#ToonShade_AntiShade}
...
### Refraction 屈折 {#Refraction}

半透明に屈折効果を組み合わせることができます。この機能を持つシェーダは `Unlit_WF_UnToon_Custom_Transparent_Refracted` です。
通常の Transparent シェーダと同じように使いながら、透けて見える景色を屈折効果で歪ませることができます。
半透明の設定は [TransparentAlpha](#TransparentAlpha) を参照してください。

![Image](./img/untoon-man-41.png)

- 屈折強度は `屈折率` と `距離` の組み合わせで調整します。
  - `屈折率` が大きいほど強い凸レンズになります。1.0 では屈折しません。1.0 未満のときは凹レンズになります。
  - `距離` が大きいほど強く歪みます。眼鏡のように対象と距離が近い場合には距離を小さくしてください。
- `色補正` を用いると屈折する像に色を付けることができます。色ガラスなどを表現するときに使います。
- `屈折補正` をチェックすると、CameraDepthTexture を用いて、手前側のメッシュを屈折に映さないよう補正処理が行われます。

### FrostedGlass すりガラス {#FrostedGlass}

半透明に”ぼかし”(ブラー)を組み合わせることができます。この機能を持つシェーダは `Unlit_WF_UnToon_Custom_Transparent_FrostedGlass` です。
通常の Transparent シェーダと同じように使いながら、透けて見える景色をぼかすことができます。
半透明の設定は [TransparentAlpha](#TransparentAlpha) を参照してください。

![Image](./img/untoon-man-42.png)

- `ブラー` にて強度を調整します。カメラがメッシュに近いほど”ぼかし”は強く作用し、その強度を最小・最大にて設定します。
- `ブラーモード` にて”ぼかし”処理の種類を設定します。
- `ブラーランダム化` を調整すると、すりガラスのようなランダム性が強くなります。
- `ブラー補正` をチェックすると、CameraDepthTexture を用いて、手前側のメッシュをぼかさないよう補正処理が行われます。
